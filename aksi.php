...
if ($mod == 'login_pemilih') {
    ...
    $row = $db->get_row("SELECT * FROM tb_pemilih WHERE nim='$user' AND pass='$pass'");
    if ($row) {
        $_SESSION['login'] = TRUE;
...
elseif ($mod == 'pencalon_tambah') {
    ...
    elseif ($db->get_results("SELECT * FROM tb_pencalon WHERE kode_pencalon='$kode_pencalon'"))
        ...
        $db->query("INSERT INTO tb_pencalon (kode_pencalon, nama_pencalon, gambar, visi,misi,prodi,semester) VALUES ('$kode_pencalon', '$nama_pencalon', '$filename', '$visi','$misi','$prodi','$semester')");
    ...
    elseif ($db->get_results("SELECT * FROM tb_pencalon WHERE kode_pencalon='$kode_pencalon' AND id_pencalon<>'$_GET[ID]'"))
        ...
        $db->query("UPDATE tb_pencalon SET kode_pencalon='$kode_pencalon', nama_pencalon='$nama_pencalon' $sql_gambar, visi='$visi',misi='$misi',prodi='$prodi',semester='$semester' WHERE id_pencalon='$_GET[ID]'");
        redirect_js("index.php?m=pencalon");
    }
...
if ($mod == 'pemilih_tambah') {
    ...
    elseif ($db->get_results("SELECT * FROM tb_pemilih WHERE nim='$nim'"))
        ...
        $db->query("INSERT INTO tb_pemilih (nim, nama_pemilih, prodi,semester, pass) VALUES ('$nim', '$nama_pemilih', '$prodi','$semester', '$pass')");
    ...
    elseif ($db->get_results("SELECT * FROM tb_pemilih WHERE nim='$nim' AND id_pemilih<>'$_GET[ID]'"))
        ...
        $db->query("UPDATE tb_pemilih SET nim='$nim', nama_pemilih='$nama_pemilih', prodi='$prodi',semester='$semester', pass='$pass' WHERE id_pemilih='$_GET[ID]'");
        redirect_js("index.php?m=pemilih");
    }
